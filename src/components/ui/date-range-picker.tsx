...
export function DateRangePicker({ startDate, endDate, onChangeStart, onChangeEnd, label, className, minDate }: Props) {
  ...
  function renderDay(day: Date, isCurrentMonth: boolean) {
    ...
    return (
      <div
        ...
        className={cn(
          'relative flex items-center justify-center h-7 text-[11px] transition-colors cursor-pointer',
      ...
      >
        <span
          className={cn(
            'relative z-10 flex items-center justify-center w-7 h-7 rounded-md transition-all duration-150',
            isSelected && 'bg-[#4F6BFF] text-white font-bold text-[10px] shadow-[0_2px_6px_rgba(79,107,255,0.3)]',
            !isSelected && !isDisabled && 'hover:bg-gray-100',
            isToday && !isSelected && 'font-bold text-[#4F6BFF] ring-1 ring-[#4F6BFF]/20',
  ...
  function renderMonth(monthDays: Date[], offset: number, _monthIdx: number, _yearIdx: number) {
    return (
      <div>
        <div className="grid grid-cols-7 mb-0.5">
          {WEEKDAYS.map((wd) => (
            <div key={wd} className="text-center text-[8px] font-bold uppercase text-gray-300 py-0.5">{wd}</div>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-y-0.5">
          {Array.from({ length: offset }, (_, i) => <div key={`e-${i}`} className="h-7" />)}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute z-50 top-full mt-2 left-0 right-0 rounded-xl border border-gray-200 bg-white shadow-[0_12px_40px_rgba(0,0,0,0.12)] p-3"
          >
            {/* Navigation */}
            <div className="flex items-center justify-between mb-2">
              <button type="button" onClick={prevMonth} className="w-7 h-7 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 flex items-center justify-center transition-colors">
              ...
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-gray-900">{MONTHS[viewMonth].slice(0, 3)} {viewYear}</span>
                <span className="text-[10px] text-gray-300">|</span>
                <span className="text-xs font-bold text-gray-900">{MONTHS[month2].slice(0, 3)} {year2}</span>
              </div>
              <button type="button" onClick={nextMonth} className="w-7 h-7 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 flex items-center justify-center transition-colors">
            ...
            <div className="flex items-center gap-1.5 mt-3 pt-2.5 border-t border-gray-100 flex-wrap">
              <span className="text-[9px] text-gray-400 shrink-0">Snel:</span>
              {[
                ...
                <button
                  ...
                  className="text-[10px] font-medium text-gray-500 hover:text-[#4F6BFF] bg-gray-50 hover:bg-blue-50 rounded-md px-2 py-0.5 transition-colors"
                >
                  {preset.label}
                </button>
              ))}
